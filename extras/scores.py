...
while 1:
    points = 0
    conn.request('GET', url='/foglamp/asset/points%2Fred/points')
    r = conn.getresponse()
    res = r.read().decode()
    red = json.loads(res)
    conn.request('GET', url='/foglamp/asset/points%2Fgreen/points')
    r = conn.getresponse()
    res = r.read().decode()
    green = json.loads(res)
    conn.request('GET', url='/foglamp/asset/points%2Fblue/points')
    r = conn.getresponse()
    res = r.read().decode()
    blue = json.loads(res)
    conn.request('GET', url='/foglamp/asset/points%2Faccelerometer/points?limit=50')
    r = conn.getresponse()
    res = r.read().decode()
    linear = json.loads(res)
    conn.request('GET', url='/foglamp/asset/points%2Flateral/points?limit=50')
    r = conn.getresponse()
    res = r.read().decode()
    lateral = json.loads(res)
    conn.request('GET', url='/foglamp/asset/points%2Fflip/points')
    r = conn.getresponse()
    res = r.read().decode()
    flip = json.loads(res)
